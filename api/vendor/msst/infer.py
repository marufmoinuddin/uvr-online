# coding: utf-8
#
# Vendored from MSST-WebUI (utils/utils.py) — https://github.com/ZFTurbo/Music-Source-Separation-Training
# Author: Roman Solovyev (ZFTurbo). Kept here so the app can run checkpoints
# that audio-separator cannot load (notably the HyperACE BS-RoFormer variants).
#
# Changes from upstream:
#   * stdlib logging instead of MSST's utils.logger
#   * omegaconf import made optional (only htdemucs needs it)
#   * get_model_from_config limited to the architectures vendored alongside it
#   * sdr/si_sdr/bleed_full/get_metrics (evaluation helpers) removed
...
def get_model_from_config(model_type, config_path):
	...
	if model_type == "mel_band_roformer":
		...
		# Only the architectures vendored with this app are available here.
		# Anything else is left to audio-separator.
		raise ValueError(
			f"Architecture '{model_type}' is not available in the bundled "
			"inference code."
		)
